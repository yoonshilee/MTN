...
- 训练记录：
  ...
  - 质量结论：本轮实验整体较为成功。生成结果虽然有一定模糊，但主体很容易识别为“老虎”；视频多视角连贯性很好，未见明显断裂或突变，说明整体三维一致性较强。与此同时，结果仍存在若干局部缺陷：服装部分不够自然，更像一块围裙披在身前；正面视角头部出现了两个部分重叠的头，属于典型的多头/Janus 类伪影。
  - 与 `trial_perpneg_if_tiger_baseline_6000` 的对比结论：本轮在移除 `perpneg` 与 `negative_w=-3.0` 后，不仅训练稳定性明显更好、耗时显著下降，而且最终视频质量也明显优于上一轮“球体化失败”结果。该对比可支持一个较强判断：上一轮失败与 `perpneg` 强约束设置高度相关；去掉该设置后，模型至少能够生成可辨认主体，并保持较好的多视角连贯性。
