...
\title[Failure-Aware Text-to-3D Generation with MTN]{Failure-Aware Text-to-3D Generation with MTN:\\An Experimental Study on a Tiger Doctor Prompt}
...
\keywords{text-to-3D, diffusion guidance, MTN, failure analysis, hyper-parameter study}
...
\centering
\small
\begin{tabular}{p{3.3cm} p{3.7cm} p{1.7cm} p{1.7cm} c c c p{3.1cm}}
\toprule
Workspace / name & Key setting & Time & Peak VRAM & Geo. & Tex. & Cons. & Observation \\
\midrule
\texttt{trial\_if\_lowmem} & IF, \texttt{vram\_O}, $48\times48$ & 17.77 min & 12.0 GB & 2 & 1 & 2 & Stable but blurry; object barely recognizable. \\
\texttt{trial\_if\_lr3e4} & IF, \texttt{vram\_O}, $48\times48$, \texttt{lr=3e-4} & 17.44 min & 12.0 GB & 2 & 2 & 2 & Slightly clearer; still shows strong artifacts and semantic drift. \\
\texttt{trial\_if\_perpneg} & IF, \texttt{perpneg}, default $64\times64$ & early fail & OOM & -- & -- & -- & Training terminates before usable output. \\
\texttt{trial\_if\_perpneg\_lowmem} & IF, \texttt{perpneg}, \texttt{vram\_O}, $48\times48$ & 103.98 min & 13.8 GB & 1 & 1 & 1 & Very slow; output degenerates into volumetric noise. \\
\texttt{trial\_perpneg\_if\_tiger\_baseline\_6000} & DSLR prompt, \texttt{perpneg}, \texttt{negative\_w=-3.0}, $48\times48$ & 69.62 min & 13.8 GB & 1 & 1 & 1 & Geometry collapses into a glowing sphere. \\
\texttt{exp017\_if\_tiger} & DSLR prompt, no \texttt{perpneg}, \texttt{vram\_O}, $64\times64$ & 21.78 min & 17.6 GB & 4 & 4 & 4 & Best overall result; recognizable tiger doctor with coherent views. \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{figure*}[t]
    ...
    \caption{Best result from \texttt{exp017\_if\_tiger}, referred to in the plan as \texttt{trial\_if\_tiger\_no\_perpneg\_64}. The model produces a recognizable tiger doctor with a readable lab coat silhouette and substantially better multi-view consistency than the other runs.}
...
The default IF + \texttt{perpneg} run at $64\times64$ failed before producing any render. This is not a visual failure but a system failure: the pipeline could not finish enough optimization to output a usable frame. Therefore, the correct way to report this case is \textbf{not} to fabricate a screenshot. Instead, the report should explicitly state that no image exists and use log evidence as the primary artifact.

\begin{figure}[t]
\centering
\fbox{%
\parbox{0.93\linewidth}{%
\textbf{F1 OOM evidence.}\\
Command: \texttt{python main.py -O --text "a tiger dressed as a doctor" --workspace trial\_if\_perpneg --iters 6000 --batch\_size 1 --IF --perpneg}\\
Observed symptom: training aborts before the first evaluation with \texttt{torch.OutOfMemoryError}.\\
Supporting memory trace: a closely related $64\times64$ IF + \texttt{perpneg} run with \texttt{--vram\_O} already reaches \texttt{20.5GB} GPU usage in the provided log excerpt, which explains why the non-optimized variant exceeds the memory budget.\\
Report treatment: cite logs and memory evidence rather than a non-existent render.
}}
\Description{A boxed text summary describing the out-of-memory failure, including the failed command, the early termination symptom, and the supporting 20.5GB GPU memory evidence from the logs.}
\caption{Recommended presentation of the OOM case. Because no valid image was produced, F1 should be documented as log-based evidence rather than a visual figure.}
\label{fig:f1-oom}
\end{figure}

This distinction matters academically. A figure is appropriate only when the system produced a visual artifact. For OOM, the evidence chain is command configuration, runtime interruption, and memory trace. In this project, the \texttt{20.5GB} observation is particularly useful because it shows that even the optimized variant left very little headroom on a 24 GB-class GPU.
...
\begin{figure}[t]
    ...
    \caption{F2 failure case from \texttt{trial\_perpneg\_if\_tiger\_baseline\_6000}. The model no longer represents a tiger doctor and instead collapses into a glowing volumetric sphere.}
    \label{fig:f2-collapse}
\end{figure}

The contrast between F2 and the best run suggests that the prompt itself was not the core problem. The decisive factor was the guidance strategy. For this prompt, adding strong perpendicular negative guidance over-constrained the optimization and suppressed stable object formation.
...
Third, removing a harmful constraint can be more effective than adding stronger control. The strongest positive result came from deleting \texttt{perpneg} in the DSLR-prompt run. This change improved semantics, geometry, and consistency simultaneously, while also reducing runtime relative to the DSLR + \texttt{perpneg} comparison.
